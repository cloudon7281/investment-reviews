#!/usr/bin/env python3
"""Merge PR, create semver tag, and close linked issue on approval."""
...
if re.search(r'\*\*Fixed in:\*\*\s*\(pending\)', old_body):
    new_body = re.sub(
        r'\*\*Fixed in:\*\*\s*\(pending\)', f'**Fixed in:** {next_tag}', old_body)
else:
    new_body = f'**Fixed in:** {next_tag}\n\n' + old_body

new_body = new_body.rstrip() + (
    f'\n\n---\n**{ts} — auto-closed by auto-tag-on-approval**\n'
    f'Merged PR #{PR_NUM} and tagged {next_tag}.\n'
)
